...
public class MyBatisDao<T extends IdEntity> extends SqlSessionDaoSupport implements IMybatisDao<T> {
    ...
    public T get(final Long id) {
        ...
        T obj = mapper.selectByPrimaryKey(id);
        return obj;
    }
    ...
    public List<T> get(final String[] ids) {
        ...
        Mapper<T> mapper = (Mapper<T>)getMapper(getMapperClass());
        List<T> obj = mapper.findBatchIds(ids);
        return obj;
    }
    ...
    public boolean update(final T entity) throws ConcurrentModificationException {
        ...
        if (count == 0) {
            ...
            int version = o == null ? 0 : o.getVersion();
            if (version != entity.getVersion()) {
                throw new ConcurrentModificationException(mapper.getClass().getName() + ", Stale data: version="
                    + entity.getVersion() + ", version in db=" + version);
            }
    ...
    @Override
    public List<T> findBatchIds(String[] ids) {
        if (ids == null) {
            return null;
        }
        return get(ids);
